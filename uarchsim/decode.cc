...
//      	    break;
		...
		switch (inst.opcode()) {
			...
			case OP_LOAD:
				...
				// Fix for sjeng assertion:
				// After a certain mispredicted branch, the benchmark "sjeng" fetches a bogus instruction outside
				// of the text segment without a fetch exception.
				// If the bogus instruction has x0 as a destination register, its destination register gets invalidated
				// later on in this decode function (C_valid = false).
				// If the bogus instruction is interpreted as a load, an assertion fires in the load replay logic
				// due to not having a destination register.
