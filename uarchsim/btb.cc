...
//    For each instruction slot in the fetch bundle, the BTB sets five fields (valid, branch, branch_type, pc, next_pc) and
...
void btb_t::update(uint64_t pc, uint64_t pos, insn_t insn) {
   ...
   // The entry's payload:
   btb[btb_bank][set][way].branch_type = decode(insn, (pc + (pos << 2)), target);
   btb[btb_bank][set][way].target = target;
}
