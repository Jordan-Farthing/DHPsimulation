...
void pipeline_t::alu(unsigned int index) {
	...
	//DOUBLE_WORD a, b, r;
        ...
        int rm;  // Rounding mode, for softfloat.
...
//
    ...
    case OP_OP_IMM_32:
        //DHP FIX
        //if mux is true then this instruction is for CMOVE, make execution based on that.
        if(PAY.buf[index].mux){
            if(PAY.buf[index].D_value.dw){
                PAY.buf[index].C_value.dw=PAY.buf[index].A_value.dw;
            }
            else{
                PAY.buf[index].C_value.dw=PAY.buf[index].B_value.dw;
            }
        }
        else {
            switch (inst.funct3()) {
                ...
                case FN3_SR:
                    ...
                    if (inst.funct7() == FN7_SRL) {
                        ...
                        PAY.buf[index].C_value.sdw = sext32(
                                (int32_t) PAY.buf[index].A_value.w[0] >> (inst.shamt() & 0x1f));
                    }
                        //Undefined instruction
                    else {
                        assert(0);
          ...
          case FN5_FCVT_FP2I:
            switch(inst.rs2()){
		          case RS2_FCVT_W:
		          	...
		          	//
				rm = inst.rm();
                                if (rm == 7) rm = this->get_state()->frm;
                                if (rm > 4) assert(0);
                                //if (rm > 4) throw trap_illegal_instruction();
                                softfloat_roundingMode = rm;

		          	PAY.buf[index].C_value.sdw    = sext32(f32_to_i32(PAY.buf[index].A_value.dw, rm, true));
		          	break;
          ...
          case FN5_FCVT_FP2I:
            switch(inst.rs2()){
				...
				rm = inst.rm();
		          	...
		          	PAY.buf[index].C_value.sdw = f64_to_i64(PAY.buf[index].A_value.dw, rm, true);
		          	break;
