...
void pipeline_t::rename2() {
   unsigned int i;
   unsigned int index;
   unsigned int count_checkpoint=0;
   unsigned int count_destination=0;
   ...
   if (REN->stall_branch(count_checkpoint)){
      return;
   }
   if (REN->stall_reg(count_destination)){
      return;
   }
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if(PAY.buf[index].A_valid){
         PAY.buf[index].A_phys_reg = (unsigned int)REN->rename_rsrc(PAY.buf[index].A_log_reg);
      }
      if(PAY.buf[index].B_valid){
         PAY.buf[index].B_phys_reg = (unsigned int)REN->rename_rsrc(PAY.buf[index].B_log_reg);
      }
      if(PAY.buf[index].D_valid){
         PAY.buf[index].D_phys_reg = (unsigned int)REN->rename_rsrc(PAY.buf[index].D_log_reg);
      }
      if(PAY.buf[index].C_valid){
         PAY.buf[index].C_phys_reg = (unsigned int)REN->rename_rdst(PAY.buf[index].C_log_reg);
      }
      ...
      if(PAY.buf[index].checkpoint){
         PAY.buf[index].branch_ID=(unsigned int)REN->checkpoint();
      }
