...
void fetchunit_t::fetch1(cycle_t cycle) {
   ...
   if (!tc_hit) {
      ...
      //DHP FIX
      //if in IDLE state, then can lookup in BTB as normally would
      if (!ic_miss&&(state_machine==IDLE)) {
          ...
          btb.lookup(pc, cb_predictions, ib_predicted_target, ras_predicted_target, fetch_bundle, &update,
                     ideal_table->branch_PC,true);
      }
      //DHP FIX
      //otherwise do call btb lookup but ignore branches and set all instructions sequential with proper variable set.
      else if(!ic_miss){
          btb.lookup(pc, cb_predictions, ib_predicted_target, ras_predicted_target, fetch_bundle, &update,
                     ideal_table->branch_PC,false);
      }
   }
...
bool fetchunit_t::fetch2(pipeline_register DECODE[]) {
   ...
   // - "misfetched" bundle: See discussion below regarding BTB-missed branches.
   //////////////////////////////////////////////////////////////////////////////////

   // "misfetched" bundle discussion:
   //
   // If the fetch bundle came from the instruction cache, check if the BTB missed any branches in the fetch bundle.
   // If one or more branches were missed, the fetch bundle is potentially malformed with respect to the "intent"
   ...
   while ((pos < instr_per_cycle) && FETCH2[pos].valid) {
	 ...
	 //case OP_MISC_MEM:  // FIX_ME: Make fence a serializing instruction, add fence flag to Active List, and reference this flag to full squash at Retire stage.
	    ...
	    // Whether the fetch bundle came from the instruction cache or trace cache, this instruction should not have been identified as a branch.
	    assert(!PAY->buf[index].branch);
	    break;
	    ...
	    else if (!PAY->buf[index].branch) {
	       // The fetch bundle came from the instruction cache and this branch was missed by the BTB.
	       misfetch = true;
   ...
   if (misfetch) {
      ...
      // DHP FIX
      //set state machine back to IDLE after restore a mispredicted branch before DHP PC
      state_machine=IDLE;
