...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      load_flag = (bool)IS_LOAD(PAY.buf[index].flags);
      store_flag = (bool)IS_STORE(PAY.buf[index].flags);
      branch_flag = (bool)IS_BRANCH(PAY.buf[index].flags);
      amo_flag = (bool)IS_AMO(PAY.buf[index].flags);
      csr_flag = (bool)IS_CSR(PAY.buf[index].flags);
    ...
    //this function will set these two fields
